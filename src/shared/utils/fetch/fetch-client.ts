...
export class FetchClient {
    ...
    private createSerachParams(params: TypeSearchParams) {
        const searchParams = new URLSearchParams()
        for (const key in { ...this.params, ...params }) {
            if (Object.prototype.hasOwnProperty.call(params, key)) {
                const value = params[key]
                if (Array.isArray(value)) {
                    value.forEach((currentValue) => { if (currentValue) { searchParams.append(key, currentValue.toString()) } })
                } else if (value) {
    ...
    private async request<T>(endpoint: string, method: RequestInit['method'], options: RequestOptions = {}) {
        ...
        if (options.params) {
            url += this.createSerachParams(options.params)
        }
    ...
    public put<T>(endpoint: string, options: RequestOptions = {}, body?: Record<string, any>) {
